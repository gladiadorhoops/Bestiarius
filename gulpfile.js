...
const paths = {
  ...
  angular_src: `${angularApp}/dist/**/*`,
  angular_dist: `prod-build/${angularApp}/dist`,
...
function installAngularDependencies(cb) {
  ...
  return exec(`cd ${angularApp} && npm install`, function (err, stdout, stderr) {
    log(stdout);
    log(stderr);
    cb(err);
  })
...
function buildAngularCodeTask(cb) {
  ...
  return exec(`cd ${angularApp} && npm run build`, function (err, stdout, stderr) {
    log(stdout);
    log(stderr);
    cb(err);
  })
